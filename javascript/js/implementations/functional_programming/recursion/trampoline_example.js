...
var sumTrampolined = trampoline(function f(sum, num, ...nums) {
    sum += num;
    if (nums.length === 0) { //Baseline / ending condition for the trampoline util
        return sum;
    } else {
        return function() { //Continuation function
...
function trampoline(fn) {
    return function trampolined(...args) {
        ...
        while (typeof result == "function") { //Keep looping if result is continuation function
            result = result();
        }
